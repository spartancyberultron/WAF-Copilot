...
def fetch_ai_cves(limit=50, api_key=None, save_file="ai_cves.json"):
    """
    Fetch confirmed & analyzed AI/ML related CVEs from NVD and save them to a JSON file.
    ...
    for kw in keywords:
        ...
        for item in data.get("vulnerabilities", []):
            ...
            if not cvss:
                continue  # skip unscored or unconfirmed CVEs
    ...
    if save_file:
        ...
        print(f"✅ Saved {len(cves[:limit])} CONFIRMED AI-related CVEs to {save_file}")
    
    return cves[:limit]


if __name__ == "__main__":
    results = fetch_ai_cves(limit=200)
    for r in results[:5]:
